...
# ==========================================
AUDIO_FILE = 'jet2.mp3' 
OUTPUT_BASE = 'neon_geometry_maximized' 
...
FPS = 60              # 60 FPS for fluid motion
...
def generate_manifold_video(audio_path):
    ...
    # Remove all margins so the plot touches the edges of the video
    plt.subplots_adjust(left=0, right=1, bottom=0, top=1)
    
    ax = fig.add_subplot(111, projection='3d', facecolor=BACKGROUND_COLOR)

    # --- ZOOM CAMERA ---
    # Default dist is approx 10. Reducing it zooms IN.
    ax.dist = 7  
    ...
    # Title - Moved slightly to not be cut off by zoom
    ax.set_title(f'S O N I C   G E O M E T R Y   ::   {audio_path}', 
                 color='white', fontsize=14, fontname='Consolas', pad=10, alpha=0.8, y=0.98)
    ...
    # Adjusted position (0.05, 0.85) to be safe within the zoomed view
